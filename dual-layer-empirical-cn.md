...
本文通过四组实验——六组 API 行为实验加一组内部注意力模式分析——提供了大语言模型（LLM）以两种质性不同的神经通路处理结构化格式与自然语言的实证证据。实验一（结构-内容分离，DeepSeek-V3）证明结构正确性与语义内容完全可分离——无意义内容不影响结构输出，非法结构不影响语义生成，且 JSON 和 XML 表现一致。实验二（噪声注入鲁棒性，DeepSeek-V3）证明在结构化格式内部注入多达 156 词的无关自然语言文本后，模型仍能 100% 恢复原始结构，且不在输出中提及噪声的存在——表明结构恢复由下层自动回路完成，未经上层意识参与。实验三（格式切换代价，DeepSeek-V3）发现进入结构格式的过渡是干净的（100%），但退出时存在惯性（JSON OUT=0%，XML OUT=50%），揭示了闭合标签的显式程度影响回路切换代价。实验四（注意力模式分析，Llama-3.3-70B-INT8 + Qwen2.5-72B-AWQ）直接观察模型内部激活，发现 JSON 处理激活集中在浅层（前 20%）的少数专用注意力头（占总头数 2-3%），与负责语义处理的深层分离；该模式在两个不同家族、不同量化方法的模型中一致。跨格式对比证明该结构回路是**格式无关的**——它识别的是拓扑结构（嵌套、配对），而非特定语法符号。跨模型对比证明该回路是 **Transformer 架构的涌现特性**而非特定训练管线的产物。这些结果为双层神经架构假说提供了首批多模型、多方法的实证证据：LLM 中存在一个独立于语义处理的"结构运动回路"，其功能类比于生物系统中的程序性记忆。
...
1. **"运动回路"是类比**：我们不声称 LLM 拥有生物运动神经元。该类比用于描述功能特性（自动性、耐噪声性、不可自省性）。
2. **噪声实验未触发相变**：JSON 77 词、XML 156 词均未触发崩溃，DeepSeek-V3 的结构回路容量极高，需更极端条件寻找临界点。
...
本文通过四组实验——六组 API 行为实验（JSON 与 XML 各三组，被试：DeepSeek-V3）加一组内部注意力模式分析（被试：Llama-3.3-70B、Qwen2.5-72B）——提供了双层神经架构假说的首批多模型、多方法实证证据。我们证明了：
...
这些发现支持了以下观点：LLM 内部存在功能上可区分的两个处理层——一个自动且面向格式（浅层专用头），一个审慎且面向意义（深层语义头）——类比于生物认知中程序性记忆与陈述性记忆的区分。跨格式、跨模型的一致性表明这不是针对特定训练数据的过拟合，而是 Transformer 架构中涌现的通用结构处理能力。
