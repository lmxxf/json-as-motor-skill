...
本文通过三组行为实验，提供了大语言模型（LLM）以两种质性不同的神经通路处理结构化格式（JSON）与自然语言的实证证据。实验一（结构-内容分离）证明 JSON 的结构正确性与语义内容完全可分离——无意义内容不影响结构输出，非法结构不影响语义生成。实验二（噪声注入鲁棒性）证明在有效 JSON 内部注入多达 77 词的无关自然语言文本后，模型仍能 100% 恢复原始结构，且不在输出中提及噪声的存在——表明结构恢复由下层自动回路完成，未经上层意识参与。实验三（格式切换代价）发现从自然语言进入 JSON 的过渡是干净的（100%），但从 JSON 退出到自然语言的过渡是混乱的（0%干净率），揭示了 JSON 解析回路的"惯性"特征。这些结果为双层神经架构假说提供了首批行为学证据：LLM 中存在一个独立于语义处理的"结构运动回路"，其功能类比于生物系统中的程序性记忆。

**关键词**：大语言模型，双层架构，程序性记忆，JSON解析，结构运动回路，DeepSeek-V3
...
|------|-----------|---------------------|
| 实验一 | 结构与语义使用不同通路 | 上层处理语义，下层处理结构 |
| 实验二 | 结构回路自动运行，不经上层意识 | 下层不可自省 |
| 实验三 | 回路切换有方向性代价 | 两层是不同的"电路"，切换需要时间 |

### 5.2 运动回路 vs 认知计算

我们的核心论点是：LLM 的 JSON 处理更像"运动技能"而非"认知任务"。证据对照：

| 运动技能特征 | 人类例子 | LLM JSON 处理的对应证据 |
|-------------|---------|----------------------|
| 对内容变化鲁棒 | 不管打什么字，十指打字的肌肉动作一样 | 实验一：无意义内容不影响结构正确性 |
| 对干扰耐受 | 运动员在嘈杂环境中仍能完成动作 | 实验二：77 词噪声全部恢复 |
| 不需要意识参与 | 熟练驾驶不需要"想怎么转方向盘" | 实验二：0% 上层意识参与 |
| 有切换惯性 | 从打字切换到写字有短暂不适 | 实验三：OUT 过渡 0% 干净 |
| 激活后自动执行 | 一旦开始打字就停不下来 | 实验三：IN = 100%，激活即可靠 |
...
2. **单一模型**：仅在 DeepSeek-V3 上验证。后续需在 Claude/GPT/LLaMA 上重复。
3. **"运动回路"是类比**：我们不声称 LLM 拥有生物运动神经元。该类比用于描述功能特性。
4. **噪声实验未触发相变**：DeepSeek-V3 的结构回路可能容量极高，需更极端条件。
...
本文通过三组行为实验提供了双层神经架构假说的首批实证证据。我们证明了：

1. LLM 的结构处理和语义处理是可分离的独立通路
2. 结构处理回路具有"运动技能"特征：自动、耐噪声、不可自省
3. 两个回路之间存在方向性的切换代价

这些发现支持了以下观点：LLM 内部存在功能上可区分的两个处理层——一个自动且面向格式，一个审慎且面向意义——类比于生物认知中程序性记忆与陈述性记忆的区分。
